...
async function downloadJSAgent(_config){
	let dynatraceConfig = _config;

	try{
		if(dynatraceConfig === undefined){
			dynatraceConfig = await config.readSettings();
		}

		await _downloadJSAgent(dynatraceConfig);
	}catch(exception){
		...
		if(dynatraceConfig.generalConfig && dynatraceConfig.generalConfig.autoUpdate){
			// Build Should fail
			logger.logMessageSync("Stopping the build because AUTO_UPDATE is true and download of JSAgent failed!", logger.ERROR);
			throw new Error("Stopping the build because AUTO_UPDATE is true and download of JSAgent failed!");
		}else{
			// Check if JSAgent file is available
			return files.checkIfFileExists(paths.getDownloadJSAgentPath())
			.catch(() => {
				throw new Error("Stopping the build because AUTO_UPDATE is false and there is no pre downloaded JSAgent!");
			});
		}
	}
}
...
async function _downloadJSAgent(configs){
	...
	// Check if AutoUpdate
	if(configs.generalConfig && configs.generalConfig.autoUpdate){
...
async function downloadJSAgentHttp(downloadConfig, type){
	...
	if(type == config.DYNATRACE_SAAS || type == config.DYNATRACE_MANAGED){
		...
		let content = await createHTTPRequestWithRetries(3, options, "Download of agent options finished", "Could not download agent options");
		...
		// App Mon
		await createHTTPRequestWithRetries(3, options, "Download of agent file finished", "Could not download agent file: ", path.resolve(paths.getDownloadJSAgentPath()));
	}
}
...
/**
 * Downloading JSAgent with Retries
 ...
 * @returns {void}
 */
async function createHTTPRequestWithRetries(retries, options, finishMsg, errorMsg, destFile){
	...
	do{
		...
		try{
			content = await exports.createHTTPRequest(options, finishMsg, errorMsg, destFile);
		}catch(error){
